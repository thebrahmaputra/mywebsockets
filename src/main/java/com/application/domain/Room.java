...
public class Room implements Function <String, Room> {
    ...
    public synchronized void leave (Session session){
        sendMessage(new Message("Duke chatbot ", format("%s left the chat room ", (String) session.getUserProperties().get("userName"))));
        sessions.remove(session);
    }

    public void sendMessage(Message message){
        sessions.parallelStream().filter(Session::isOpen)
